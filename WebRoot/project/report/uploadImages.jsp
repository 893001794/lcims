...
<h2 align="center"><b>上传报告图片</b></h2>
<form id="uploadForm" action="uploadImages_post.jsp?sid=<%=p.getSid() %>" method="post" ENCTYPE="multipart/form-data">
	<input name="sid" type="hidden" value="<%=p.getSid() %>" />
<table border="0" cellspacing="1" class="fu_list" align="center">
    <thead>
      <tr>
        <td>报告编号：</td>
        <td><font size="3" style="font-weight: bolder"><%=p.getRid() %></font></td>
      </tr>
    </thead>
    <tbody>
      <tr>
        <td align="right" width="15%" style="line-height:35px;">添加图片：</td>
      ...
      <tr>
        <td colspan="2" align="center" id="idMsg"><input type="button" value="开始上传" id="idBtnupload" disabled="disabled" />
          &nbsp;&nbsp;&nbsp;
          <input type="button" value="全部删除" id="idBtndel" disabled="disabled" />
          &nbsp;&nbsp;&nbsp;
...
onIni: function(){
   ...
   if(this.Files.length){
    ...
    Each(this.Files, function(o){
     var a = document.createElement("a"); a.innerHTML = "删除"; a.href = "javascript:void(0);";
...
$("idMsg").innerHTML = "正在上传图片，请稍候……<br />有可能因为网络问题，出现程序长时间无响应，请点击“<a href=''><font color='red'>取消</font></a>”重新上传文件";
...
alert(msg);
location.href = "../../chemistry/project/myproject.jsp";
}
