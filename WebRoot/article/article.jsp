...
. {
	border: solid 1px;
		...
		function checkForm() {
			...
			if(document.getElementById("nextcal").value == ""){
			alert("进来");
			alert("复校准时间不能为空!");
			return false;
			}
			if(document.getElementById("calibration").value == ""){
			alert("校准时间不能为空!");
			return false;
			}
		}
	...
	<body class="body1">
		...
		<form name="form1" action="article_post.jsp?status=ad" method="post" onsubmit="return  checkForm();">
			<div class="">
				...
				<!-- addarticle.jsp -->
					...
					<a href="javascript:void(0)" onclick="window.open(' addarticle.jsp','child','width=600,height=200,left=400,top=400');">
					添加物品名称
					</a></td>
				</tr>
					<tr>
					...
					<td>
						...
						<td >
								<input name="specification" size="40" value=""    />
						</td>
						<td>
							数量：
						</td>
						<td>
							<input name="number" size="40" value=""    />
						</td>
					</tr>
					...
					<tr>
						...
						<td>
							<input name="price" size="40" value=""    />
						</td>
						<td>
							供应商：
						</td>
						...
						<td>
							<select name="c2" style="width: 300px">
								...
								for(String value:classMap2.keySet()) {
								 %>
								<option value="<%=classMap2.get(value) %>" <%=classMap2.get(value).equals("PCS") %>><%=classMap2.get(value) %></option>
								<%
								 }
							...
							<select name="c3" style="width: 300px">
								...
								for(String value:classMap3.keySet()) {
								 %>
								<option value="<%=classMap3.get(value) %>" <%=classMap3.get(value).equals("使用中")?"selected":"" %>><%=classMap3.get(value) %></option>
								<%
								 }
					...
					<tr>
						<td>使用年限:</td>
						<td>
						<input name="usedate" type="text" id="usedate" size="40" />
							...
							<select name="c5" style="width: 300px">
								...
								for(String value:classMap5.keySet()) {
								 %>
								<option value="<%=classMap5.get(value) %>"  <%=classMap5.get(value).equals("中山立创")?"selected":"" %>><%=classMap5.get(value) %></option>
								<%
								 }
					...
					<tr>
						<td>保管人:</td>
						<td>
						<input name="keeper" type="text" id="keeper" size="40" />
						...
						<td>
							<input name="nextcal" type="text" id="quotime" size="40" />
					...
					<tr>
						...
						<td>
							<input name="calibration" type="text" id="quotime" size="40" />
						...
						<td>
							<input name="contract" size="40" value=""    />
						</td>
					</tr>
					<tr>
					<td> 
						...
						<td>
							<input name="invoicecode" size="40" value=""    />
						</td>
						<td> 
							发票号码： 
						</td>
						<td>
							<input name="invoiceno" size="40" value=""    />
						</td>
					</tr>
					  <tr>
							<td>
							...
							<td colspan="3">
								<textarea name="notes" cols="120" rows="6"></textarea>
							</td>
						</tr>
				</table>
			</div>
