...
WebFXTreeItem.prototype.toString = function (nItem, nItemCount) {
	...
	var itemNumber;
	if(label.indexOf("|")>-1){
		 label1 = label.substring(0, label.indexOf("|")); // 取子字符串。
		 itemNumber= label.substring(label.indexOf("|")+1,label.length); // label.length()取子字符串。
	}else{
		label1=label;
	}
	...
	var str = "<div id=\"" + this.id + "\" ondblclick=\"webFXTreeHandler.toggle(this);\" class=\"webfx-tree-item\" onkeydown=\"return webFXTreeHandler.keydown(this, event)\">" +
		...
		"<img id=\"" + this.id + "-icon\" class=\"webfx-tree-icon\" src=\"" + ((webFXTreeHandler.behavior == 'classic' && this.open)?this.openIcon:this.icon) + "\" onclick=\"webFXTreeHandler.select(this);\">" +
		"<a href=\"phpInfo.jsp?itemNumber="+itemNumber+"\" id=\"" + this.id + "-anchor\" onfocus=\"webFXTreeHandler.focus(this);\" onblur=\"webFXTreeHandler.blur(this);\"" +
		("target=\"phyp\"") +
		">" + label1 + "</a>" +
		"</div>"+
