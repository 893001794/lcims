...
<%
	...
	TestChildParent tp =new TestChildParent();
	tp =ChemTestDaoImpl.getInstance().findChildById(Integer.parseInt(id));
		...
		<hr width="97%" align="center" size=0>
			...
			<tr>
					...
					<td width="33%">
						...
						<input name="chemtestName" type="text" size="40" value="<%=tp.getChildName()%>"<%=tp.getChildName() ==null?"":tp.getChildName() %>/>
					</td>
					<td width="17%">
						类别：
					</td>
			...
			<table>
				<tr>
					...
					<%
					List list =ChemTestDaoImpl.getInstance().findChemTestParent(tp.getType());
					for(int i=0;i<list.size();i++){
					
					TestParents tps =(TestParents)list.get(i);
					if(tp.getParentName().equals(tps.getName())){
					%>
					<option value="<%=tps.getId() %>" <%=tps.getName()==null?"":"selected" %>><%=tps.getName() %>&nbsp;&nbsp;<%=tps.getType() %></option>
					<%
					}else{
					%>
					<option value="<%=tps.getId() %>" ><%=tps.getName() %>&nbsp;&nbsp;<%=tps.getType() %></option>
					<%
					}
