...
public class FinanceExport extends HttpServlet {
	...
	public synchronized void exportProject(HttpServletRequest request,HttpServletResponse response) {
		...
		String flowfile = path + "export" + fs + "finance.xls";
		...
		try {
			...
			for(int i=0;i<list.size();i++) {
				...
				row.createCell(18).setCellValue((qt.getInvtype().equals("收据")?"0":qt.getTotalprice()*0.08)+"");
				...
				row.createCell(23).setCellValue(qt.getInvtype().equals("收据")?"0":(qt.getPrespefund()*scale)+"");
				...
				row.createCell(28).setCellValue(qt.getFinish()!=null?"结案":"进行中");
				row.createCell(29).setCellValue(qt.getFinish()==null?"":qt.getFinish()+"");
			}
//			row = sheet.getRow(1);
//			row.createCell(30).setCellValue(f[1]);
//			row.createCell(31).setCellValue(f[0]);
			fOut = new FileOutputStream(new File(flowfile));
			wb.write(fOut);
			response.sendRedirect("export/finance.xls");
		} catch (Exception e) {
			e.printStackTrace();
		} finally {
			if(fOut != null) {
